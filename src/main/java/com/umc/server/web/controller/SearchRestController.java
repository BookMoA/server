...
public class SearchRestController {
    ...
    @Operation(
            summary = "책리스트 검색 API",
            description =
                    "책리스트 제목을 검색하는 API입니다. sortBy로 정렬 가능합니다. ['newest' - 최신순, 'oldest' - 오래된 순, 'relevance' - 정확도 순, 'rating_desc' - 별점 높은 순, 'rating_asc' - 별점 낮은 순]")
    @GetMapping("/list")
    ...
    @Operation(
            summary = "메모 검색 API",
            description =
                    "메모를 검색하는 API입니다. sortBy로 정렬 가능합니다. ['newest' - 최신순, 'oldest' - 오래된 순, 'relevance' - 정확도 순, 'rating_desc' - 별점 높은 순, 'rating_asc' - 별점 낮은 순]")
    @GetMapping("/memo")
